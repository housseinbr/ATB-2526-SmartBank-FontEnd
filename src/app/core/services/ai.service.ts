...
export interface AiChatResponse { model: string; response: string; }
...
export class AiService {
  ...
  conversationChat(message: string, conversationId: number | null): Observable<{conversationId:number;response:string}> { return this.http.post<{conversationId:number;response:string}>(`${environment.apiUrl}/ai/conversations/chat`, { message, conversationId }, { headers: this.headers() }); }
